package com.wiatec.update.utils;


public class Constant {
	
	/**
	 * true 表示需要进行mac地址校验
	 * false 表示不需要进行mac地址校验
	 */
	public static final boolean MACAUTCH = true;
	
	public static final String FILEDELTXT = "/Android/data/org.xbmc.kodi/files/.kodi/filedel.txt";
	
	public static final String VERSIONURL = "http://btvota.gobeyondtv.co/v3/update.js";
	//public static final String VERSIONURL = "http://btvota.gobeyondtv.co/v3/uptest.js";
	//http://btvota.gobeyondtv.co/v3/update.js
	
	public static final String APPUPDATEURL = "http://btvota.gobeyondtv.co/v3/APP/UPDATEAPP.js";
	//http://btvota.gobeyondtv.co/v3/APP/UPDATEAPP.js
}
